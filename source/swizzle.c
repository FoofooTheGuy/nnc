    ...
    for(y = 0; y < ydim; y += 8)                                                                        \
      for(x = 0; x < xdim; x += 8)                                                                      \
        ...
        {                                                                                               \
          ...
          conversion_routine                                                                            \
          outp[(y + sy) * xdim + x + sx] = out_colour;                                                  \
        }                                                                                               \
  }

#define DEFINE_SWIZZLE_FUNC(format_name, inp_type, outp_type, endian_func, vars, conversion_routine)  \
  void nnc_swizzle_zorder_##format_name(u32 *inp, u16 *outp, u16 xdim, u16 ydim)                      \
  {                                                                                                   \
    u16 sx, sy, y, x, block;                                                                          \
    inp_type in_colour;                                                                               \
    outp_type out_colour;                                                                             \
...
,
	r = FIX_COLOUR_DEPTH(in_colour >> 11 & 0x1F, 5, 8);
...
DEFINE_SWIZZLE_FUNC(rgba8_to_le_rgb565, u32, u16, LE16,
	...
	u8 b;
,
	r = FIX_COLOUR_DEPTH(in_colour >> 24 & 0xFF, 8, 5);
	g = FIX_COLOUR_DEPTH(in_colour >> 16 & 0xFF, 8, 6);
	b = FIX_COLOUR_DEPTH(in_colour >> 8  & 0xFF, 8, 5);
