...
typedef union nnc_vfs_or_stream_or_voidp {
	nnc_vfs     *vfs;
	nnc_rstream *stream;
	void        *voidp;
} nnc_vfs_or_stream_or_voidp;

typedef union nnc_exheader_or_stream {
	nnc_exheader *exheader;
	nnc_rstream  *stream;
} nnc_exheader_or_stream;

/** Flags for writing an ncch. */
enum nnc_ncch_wflags {
	NNC_NCCH_WF_ROMFS_VFS        = 1,  ///< Use the VFS pointer to write the RomFS.
	NNC_NCCH_WF_ROMFS_STREAM     = 0,  ///< Use the rstream pointer to copy the RomFS.
	NNC_NCCH_WF_ROMFS_OMIT       = 0,  ///< Omit the RomFS in the NCCH, pass NULL for the romfs parameter.
	NNC_NCCH_WF_EXEFS_VFS        = 2,  ///< Use the VFS pointer to write the ExeFS.
	NNC_NCCH_WF_EXEFS_STREAM     = 0,  ///< Use the rstream pointer to copy the ExeFS.
	NNC_NCCH_WF_EXEFS_OMIT       = 0,  ///< Omit the ExeFS in the NCCH, pass NULL for the exefs parameter.
	NNC_NCCH_WF_EXHEADER_WRITE   = 4,  ///< Write a new exheader based on the exheader field in the union.
	NNC_NCCH_WF_EXHEADER_STREAM  = 0,  ///< Copy the exheader from a stream.
};
 ...
 *  \param header    The header to write. Note that everything that is either\n
 *                    a) crypto related (that is, keyy, hashes, crypt_method);\n
 *                    b) an offset or size;\n
 *                    c) the flags, version and content_unit fields;\n
 *                   is not used in writing the header, they are instead generated by this function.
